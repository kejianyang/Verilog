...
![1562898490756](assets/1562898490756.png)
...
VGA  :Video Graphics Array  视频图形阵列，使用模拟信号进行视频传输的标准

![1562900523048](assets/1562900523048.png)

#### 行同步时序

![1562900719743](assets/1562900719743.png)

#### 场同步时序

![1562900828281](assets/1562900828281.png)
...
//行场信号
wire vga_hs=(cnt_h<H_SYNC-1'b1)?1'b0:1'b1;
wire vga_vs=(cnt_v<V_SYNC-1'b1)?1'b0:1'b1;

//请求数据输入
assign data_req=((cnt_h>=H_SYNC+H_BACK-1'b1)&&(cnt_h<H_SYNC+H_DISP+H_BACK+H_DISP-1'b1))&&
			   &&((cnt_v>=V_SYNC+V_BACK)&&(cnt_v<V_SYNC+V_BACK+V_DISP)))
			   ?1'b1:1'b0;


//RGB信号使能
assign vga_en=(((cnt_h>=H_SYNC+H_BACK)&&(cnt_h<H_SYNC+H_DISP+H_BACK+H_DISP))&&
			   &&((cnt_v>=V_SYNC+V_BACK)&&(cnt_v<V_SYNC+V_BACK+V_DISP)))
			   ?1'b1:1'b0;
//RGB输出
assign vga_rgb=vga_en?pixel_data:16'd0;

assign vga_xpos=data_req?(cnt_h-H_SYNC-H_BACK+1'b1):16'd0;
assign vga_ypos=data_req?(cnt_v-V_SYNC-V_BACK+1'b1):16'd0;
...
![1567410723289](Verilog学习.assets/1567410723289.png)

#### DE（数据有效）同步时序

![1567410932895](Verilog学习.assets/1567410932895.png)

![1567411071169](Verilog学习.assets/1567411071169.png)

7寸分辨率时序

时钟：1056x525x60 Hz

DE 数据有效同步时序  H V 信号赋值为1
